    ...
    B: hal::Backend,
{
    fn new(
        ...
        let buffer_memory = unsafe {
            let mut memory = device.allocate_memory(upload_type, buffer_req.size).unwrap();
        ...
        let img =
            image::load(Cursor::new(&img_data[..]), image::ImageFormat::Png).unwrap().to_rgba8();
        ...
        let image_upload_memory = unsafe {
            let mut memory = device.allocate_memory(upload_type, image_mem_reqs.size).unwrap();
        ...
        unsafe {
            ...
            device.wait_for_fence(&copy_fence, !0).expect("Can't wait for fence");
        }
        ...
        for i in 0..frames_in_flight {
            submission_complete_semaphores
                .push(device.create_semaphore().expect("Could not create semaphore"));
    ...
    fn render(&mut self) {
        ...
        unsafe {
            ...
            self.device.wait_for_fence(fence, !0).expect("Failed to wait for fence");
            self.device.reset_fence(fence).expect("Failed to reset fence");
            ...
            cmd_buffer.begin_render_pass(
                ...
                iter::once(command::RenderAttachmentInfo {
                    ...
                    clear_value: command::ClearValue {
                        color: command::ClearColor { float32: [0.8, 0.8, 0.8, 1.0] },
                    },
                }),
    ...
    B: hal::Backend,
{
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            self.device.destroy_descriptor_set_layout(ManuallyDrop::into_inner(ptr::read(
                &self.set_layout,
            )));

            self.device.destroy_buffer(ManuallyDrop::into_inner(ptr::read(&self.vertex_buffer)));
            self.device
                .destroy_buffer(ManuallyDrop::into_inner(ptr::read(&self.image_upload_buffer)));
            self.device.destroy_image(ManuallyDrop::into_inner(ptr::read(&self.image_logo)));
            self.device.destroy_image_view(ManuallyDrop::into_inner(ptr::read(&self.image_srv)));
            ...
            self.device.free_memory(ManuallyDrop::into_inner(ptr::read(&self.image_memory)));
            self.device.free_memory(ManuallyDrop::into_inner(ptr::read(&self.image_upload_memory)));
            self.device
                .destroy_graphics_pipeline(ManuallyDrop::into_inner(ptr::read(&self.pipeline)));
            self.device.destroy_pipeline_layout(ManuallyDrop::into_inner(ptr::read(
                &self.pipeline_layout,
            )));
